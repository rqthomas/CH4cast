...
ebu$full_time_day <- as.POSIXct(strptime(ebu$date, '%Y-%m-%d', tz = 'EST'))

a <- ggplot(ensemble_forecasts_ch4) +
  geom_point(data = mean_observe_all, aes(x = full_time_day, y = exp(ebullition_prediction)),pch = 23, color = "black", fill = "orange", size = 3)+
  geom_point(data = mean_forecast_ch4, aes(x = full_time_day, y = exp(ebullition_prediction)),pch = 23, color = "black", fill = "blue", size =2)+
  geom_flat_violin(data = ensemble_forecasts_ch4, aes(x = full_time_day, y = exp(ebullition_prediction), group = full_time_day), size=.1, color = "black", fill = "blue", scale = "width")+
  geom_point(data = ebu, aes(x = full_time_day, y = exp(log_ebu_rate_mg_m2_d)), pch = 21, fill = "orange", color = "black", cex = 1.2)+
  geom_point(data = mean_observe_all, aes(x = full_time_day, y = exp(ebullition_prediction)),pch = 23, color = "black", fill = "orange", size = 3)+
  xlab("")+
...
b <- ggplot(ensemble_forecasts_ch4_nDA) + 
  geom_point(data = mean_observe_all, aes(x = full_time_day, y = exp(ebullition_prediction)),pch = 23, color = "black", fill = "orange", size = 3)+
  geom_point(data = mean_forecast_ch4_nDA, aes(x = full_time_day, y = exp(ebullition_prediction)),pch = 23, color = "black", fill = "magenta", size =2)+
  geom_flat_violin(data = ensemble_forecasts_ch4_nDA, aes(x = full_time_day, y = exp(ebullition_prediction), group = full_time_day), size=.1, color = "black", fill = "magenta", scale = "width")+
  geom_point(data = ebu, aes(x = full_time_day, y = exp(log_ebu_rate_mg_m2_d)), pch = 21, fill = "orange", color = "black", cex = 1.2)+
  xlab("")+
  ylab(expression(paste("Ebullition rate (mg CH "[4]," m"^"-2"," d"^"-1",")")))+
...
ab <- a+transition_states(full_time_day, wrap = T) +
  shadow_mark()
anim_save("./figures/FORECAST_W_DA.gif", ab, width = 1000, height = 600)

bb <- b+transition_states(full_time_day, wrap = T) +
  shadow_mark()
anim_save("./figures/FORECAST_WO_DA.gif", bb, width = 1000, height = 600)
############################################################################################
...
############################################################################################
temp_eval <- cbind(mean_temp_forecasts, ebullition_1107[6:24,6])
